...
public class SeasonTicketServiceImpl implements SeasonTicketService {
	...
	public SeasonTicketBaseBean searchByBasePk(String base_pk) {
		...
		return baseDao.selectWithInfoByPrimaryKey(base_pk);
	}

	@Override
	public String deleteSeasonTicket(String base_pk) {
		// 删除套票基本信息
		baseDao.delete(base_pk);

		// 删除套票详细信息
		infoDao.deleteByBasePk(base_pk);
		return SUCCESS;
	}
	...
	public String updateSeasonTicket(String json, SeasonTicketBaseBean base) {
		...
		// 删除现存才套票信息
		infoDao.deleteByBasePk(base.getPk());
		// 保存新的套票详细信息
		JSONArray arr = JSONArray.fromObject(json);
		for (int i = 0; i < arr.size(); i++) {
			JSONObject obj = JSONObject.fromObject(arr.get(i));
