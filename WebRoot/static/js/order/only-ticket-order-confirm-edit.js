...
var OrderContext = function() {
	...
	$.getJSON(self.apiurl + 'order/searchTbcBnsOrderByPk', {
		...
		var x = new Date(self.do_confirm_date());
	...
	self.updateOrder = function() {
		...
		var do_date = new Date(self.do_confirm_date());
		...
		for (var i = 0; i < trs.length; i++) {
			...
			name_json += '{"pk":"' + pk + '","lock_flg":"' + lock_flg + '","chairman":"' + teamChairman + '","index":"'
		...
		$.ajax({
			type : "POST",
			url : self.apiurl + 'order/updateConfirmedOnlyTicketOrder',
			data : data
