...
var ProductBoxContext = function() {
	...
	self.checkConfirmPic = function(fileName, team_number) {
		...
		$("#img-pic").attr(
				"src",
				self.apiurl + 'file/getFileStream?fileFileName=' + fileName + "&fileType=CLIENT_FINAL&subFolder="
						+ team_number);
	};
	// 查看确认件
	...
	self.checkVoucherPic = function(fileName, team_number) {
		...
		$("#img-pic").attr(
				...
				self.apiurl + 'file/getFileStream?fileFileName=' + fileName
						+ "&fileType=CLIENT_FINAL_VOUCHER&subFolder=" + team_number);
	};
