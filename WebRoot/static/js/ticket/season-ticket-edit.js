...
var FlightContext = function() {
	...
	self.base_pk = $("#key").val();

	self.seasonTicket = ko.observable({});

	startLoadingSimpleIndicator("加载中...");
	$.getJSON(self.apiurl + 'ticket/searchSeasonTicketByBasePk', {
		base_pk : self.base_pk
	}, function(data) {
		self.seasonTicket(data.base);
		endLoadingIndicator();
	});
	...
	self.updateFlight = function() {
		...
		$.ajax({
			type : "POST",
			url : self.apiurl + 'ticket/updateSeasonTicket',
			data : data
		}).success(
				function(str) {
					if (str == "success") {
