var SupplierContext = function() {
	...
	self.genders = [ '男', '女' ];
	self.paymentTypes = [ '现结', '月结', '隔月', '季结', '其他', '未知' ];
	...
	$.getJSON(self.apiurl + 'supplier/searchOneSupplier', {
		...
		if (data.supplier) {
			...
			self.ter();
			$("#city").val(self.supplier().supplier_city);
		} else {
			fail_msg("公司不存在！");
		}
		endLoadingIndicator();
	...
	self.saveSupplier = function() {
		...
		$.ajax({
			...
			if (str == "success") {
				window.location.href = self.apiurl + "templates/ticket/supplier.jsp";
			}
		});
	};
};
...
function deleteImage(deleteButton, inputFile, img, fileNameInput, fileName) {
	...
	$.ajax({
		type : "POST",
		url : ctx.apiurl + 'supplier/deleteSupplierFile',
		data : "file_name=" + fileName + "&supplier_pk=" + ctx.supplierPk
	}).success(function(str) {
		if (str == "success") {
			//do nothing
		}
