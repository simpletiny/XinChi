...
ko.bindingHandlers.inlineSelect = {
    init:  function(element, valueAccessor, allBindingsAccessor, data){
        var span = $(element);
        var select = $('<select></select>', {'style' : 'display: none', 'class': 'form-control'});
...
var inlineTextFunc = function(element, valueAccessor, allBindingsAccessor, data, clazz) {
    var span = $(element);
    var input = $('<input />',{'type': 'text', 'style' : 'display:none', 'class': clazz});
    var changeFunc = allBindingsAccessor().changeFunc;
    span.after(input);
...
var addTotalCountLabel = function(idLabel, total) {
    $("#" + idLabel).html('<label style="background-color: #6495ED; font-size: smaller; padding-right: 10px;padding-left: 10px;" class="label">共&nbsp;'
                + total +'&nbsp;条结果</label>');
};
