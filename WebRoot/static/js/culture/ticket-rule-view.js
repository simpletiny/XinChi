var ViewContext = function() {
	...
	self.createView = function() {
		window.location.href = self.apiurl + "templates/culture/ticket-rule-view-creation.jsp";
	};
	...
	self.refresh = function() {
		...
		$.getJSON(self.apiurl + 'culture/searchTicketRuleViewByPage', param, function(data) {
			...
			$.layer({
				...
				dialog : {
					...
					yes : function(index) {
						$.ajax({
							type : "POST",
							url : self.apiurl + 'culture/deleteTicketRuleView',
							data : "view_pk=" + self.chosenViews()
						}).success(function(str) {
							if (str == "OK") {
								window.location.href = self.apiurl + "templates/culture/ticket-rule-view.jsp";
							}
						});
					}
				}
