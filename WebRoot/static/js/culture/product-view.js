var ViewContext = function() {
	...
	self.createView = function() {
		window.location.href = self.apiurl + "templates/culture/product-view-creation.jsp";
	};
	...
	self.refresh = function() {
		...
		$.getJSON(self.apiurl + 'culture/searchProductViewByPage', param, function(data) {
			...
			$.layer({
				...
				dialog : {
					...
					yes : function(index) {
						$.ajax({
							type : "POST",
							url : self.apiurl + 'culture/deleteProductView',
							data : "view_pk=" + self.chosenViews()
						}).success(function(str) {
							if (str == "OK") {
								window.location.href = self.apiurl + "templates/culture/product-view.jsp";
							}
						});
					}
				}
