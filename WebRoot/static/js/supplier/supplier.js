var SupplierContext = function() {
	...
	self.createSupplier = function() {
		window.location.href = self.apiurl + "templates/supplier/supplier-creation.jsp";
	};
	...
	self.editSupplier = function() {
		if (self.chosenCompanies().length == 0) {
			...
			window.location.href = self.apiurl + "templates/supplier/supplier-edit.jsp?key="
					+ self.chosenCompanies()[0];
		}
	};

	self.blockSupplier = function() {
		if (self.chosenCompanies().length == 0) {
			fail_msg("请选择财务主体！");
			return;
		} else if (self.chosenCompanies().length > 1) {
			...
			$.layer({
				...
				dialog : {
					...
					yes : function(index) {
						layer.close(index);
						startLoadingIndicator("停用中...");
						var data = "supplier_pk=" + self.chosenCompanies()[0];
						$.ajax({
							type : "POST",
							url : self.apiurl + 'supplier/blockSupplier',
							data : data
