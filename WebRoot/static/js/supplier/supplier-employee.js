var SupplierContext = function() {
	...
	self.createSupplierEmployee = function() {
		window.location.href = self.apiurl + "templates/supplier/employee-creation.jsp";
	};
	...
	self.refresh = function() {
		...
		param += "&employee.delete_flgs=N&employee.delete_flgs=Y"
	...
	self.editEmployee = function() {
		if (self.chosenEmployees().length == 0) {
			...
			window.location.href = self.apiurl + "templates/supplier/employee-edit.jsp?key="
			...
			$.layer({
				...
				dialog : {
					...
					yes : function(index) {
						...
						var data = "employee_pk=" + self.chosenEmployees()[0];
						$.ajax({
							type : "POST",
							url : self.apiurl + 'supplier/deleteEmployee',
							data : data
						}).success(function(str) {
							endLoadingIndicator();
							if (str == "success") {
								self.refresh();
							} else if (str == "existorder") {
								fail_msg("系统存在此供应商员工订单，不允许删除！");
							} else {
								fail_msg(str);
							}
						});
