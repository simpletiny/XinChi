...
var ProductContext = function() {
	...
	self.refresh = function() {
		...
		const dataPromise = $.getJSON(self.apiurl + 'product/searchProductDataForOrder', param, function(data) {
		...
		const orderPromise = $.ajax({
			...
			data.orders.forEach(function(data, index) {
				...
				new_order.contact_way = new_order.contact_way.length > 200 ? new_order.contact_way.substring(0, 200) : new_order.contact_way;
				new_orders.push(new_order);
			})
			self.sale_orders(new_orders);
		});

		const ticketPromise = $.getJSON(self.apiurl + 'product/searchProductAirTicketInfoByProductPk', {
			product_pk: self.product_pk
		}, function(data) {
			self.airTickets(data.air_tickets);
		})

		Promise.all([dataPromise, statusPromise, orderPromise, ticketPromise])
			.then(function() {
				if (self.productSuppliers().length > 0) {
					let heres = $(".table-order-here");
					heres.each(function(index, div) {
