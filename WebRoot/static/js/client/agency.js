...
var AgencyContext = function() {
	...
	self.chosenAgencys = ko.observableArray([]);
	self.isMapping = {
		'Y' : '有',
		'N' : '无',
	};
	self.agencys = ko.observable({
		total : 0,
		items : []
	});
	...
	self.isExit = ['有', '无'];
	self.provices = ['北京市', '天津市', '上海市', '重庆市', '河北省', '山西省', '辽宁省', '吉林省', '黑龙江省', '江苏省', '浙江省', '安徽省', '福建省', '江西省',
	...
	self.cityMapping = {
		...
		'安徽省' : ['合肥市', '芜湖市', '蚌埠市', '淮南市', '马鞍山市', '淮北市', '铜陵市', '安庆市', '黄山市', '阜阳市', '宿州市', '滁州市', '六安市', '宣城市',
				'池州市', '亳州市'],
		'福建省' : ['福州市', '莆田市', '泉州市', '厦门市', '漳州市', '龙岩市', '三明市', '南平市', '宁德市'],
		'江西省' : ['南昌市', '赣州市', '宜春市', '吉安市', '上饶市', '抚州市', '九江市', '景德镇市', '萍乡市', '新余市', '鹰潭市'],
		'山东省' : ['济南市', '青岛市', '淄博市', '枣庄市', '东营市', '烟台市', '潍坊市', '济宁市', '泰安市', '威海市', '日照市', '滨州市', '德州市', '聊城市',
				'临沂市', '菏泽市', '莱芜市'],
		'河南省' : ['郑州市', '开封市', '洛阳市', '平顶山市', '安阳市', '鹤壁市', '新乡市', '焦作市', '濮阳市', '许昌市', '漯河市', '三门峡市', '商丘市', '周口市',
				'驻马店市', '南阳市', '信阳市'],
		'湖北省' : ['武汉市', '黄石市', '十堰市', '荆州市', '宜昌市', '襄阳市', '鄂州市', '荆门市', '黄冈市', '孝感市', '咸宁市', '随州市'],
		'湖南省' : ['长沙市', '株洲市', '湘潭市', '衡阳市', '邵阳市', '岳阳市', '张家界市', '益阳市', '常德市', '娄底市', '郴州市', '永州市', '怀化市'],
		'广东省' : ['广州市', '深圳市', '珠海市', '汕头市', '佛山市', '韶关市', '湛江市', '肇庆市', '江门市', '茂名市', '惠州市', '梅州市', '汕尾市', '河源市',
				'阳江市', '清远市', '东莞市', '中山市', '潮州市', '揭阳市', '云浮市'],
		'广西壮族自治区' : ['南宁市', '柳州市', '桂林市', '梧州市', '北海市', '崇左市', '来宾市', '贺州市', '玉林市', '百色市', '河池市', '钦州市', '防城港市', '贵港市'],
		'海南省' : ['海口市', '三亚市', '儋州市', '三沙市'],
		'四川省' : ['成都市', '绵阳市', '自贡市', '攀枝花市', '泸州市', '德阳市', '广元市', '遂宁市', '内江市', '乐山市', '资阳市', '宜宾市', '南充市', '达州市',
				'雅安市', '广安市', '巴中市', '眉山市'],
	...
	self.createAgency = function() {
		window.location.href = self.apiurl + "templates/client/agency-creation.jsp";
	};

	self.cancel = function(agency_pk) {
		$.layer({
			area : ['auto', 'auto'],
			dialog : {
				msg : '确认注销旅游公司吗？',
				...
				yes : function(index) {
					$.ajax({
						type : "POST",
						url : self.apiurl + 'client/cancelAgency',
						data : "agency_pk=" + agency_pk
	...
	self.checkClient = function(data) {
		$.getJSON(self.apiurl + 'client/searchClientByAgencyPk', {
			agency_pk : data.pk
		}, function(data) {
			if (data.clients) {
	...
	self.editAgency = function() {
		if (self.chosenAgencys().length == 0) {
			fail_msg("请选择旅行社");
			return;
		} else if (self.chosenAgencys().length > 1) {
			fail_msg("编辑只能选中一个");
			return;
		} else if (self.chosenAgencys().length == 1) {
			window.location.href = self.apiurl + "templates/client/agency-edit.jsp?key=" + self.chosenAgencys()[0];
		}
	};
	// start pagination
